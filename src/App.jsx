    ...
    <div className="bg-slate-50 text-slate-900 overflow-x-hidden font-sans selection:bg-blue-100 selection:text-blue-600">
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <button onClick={() => setIsModalOpen(false)} className="absolute top-6 right-6 text-slate-400 hover:text-slate-900 transition" aria-label="Fechar">
                <X size={24} />
              </button>
              ...
              <form onSubmit={handleSubmit} className="space-y-4 text-left">
                ...
                <div>
                  <label htmlFor="name" className="text-xs font-black uppercase text-slate-400 ml-2">Seu Nome</label>
                  <input required id="name" name="name" type="text" placeholder="João Silva" className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 outline-none focus:border-blue-600 transition" />
                </div>

                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="whatsapp" className="text-xs font-black uppercase text-slate-400 ml-2">WhatsApp</label>
                    <input required id="whatsapp" name="whatsapp" type="tel" placeholder="(00) 00000-0000" className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 outline-none focus:border-blue-600 transition" />
                  </div>
                  <div>
                    <label htmlFor="email" className="text-xs font-black uppercase text-slate-400 ml-2">E-mail</label>
                    <input required id="email" name="email" type="email" placeholder="seu@email.com" className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 outline-none focus:border-blue-600 transition" />
                  </div>
                </div>

                <div>
                  <label htmlFor="message" className="text-xs font-black uppercase text-slate-400 ml-2">Sobre o seu projeto</label>
                  <textarea required id="message" name="message" rows="3" placeholder="Conte brevemente o que você precisa..." className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 outline-none focus:border-blue-600 transition resize-none"></textarea>
                </div>
      ...
      <section id="sobre" className="py-24 bg-white border-t border-slate-100">
        <div className="container mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center text-left">
          <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="relative">
            ...
            <motion.div whileHover={{ scale: 1.02 }} className="relative rounded-[2rem] overflow-hidden shadow-2xl border-8 border-white bg-slate-100">
      ...
      <section className="py-32 bg-white text-center overflow-hidden">
        <div className="container mx-auto px-6">
          <h2 className="text-4xl md:text-6xl font-black text-slate-900 mb-16 uppercase tracking-tighter italic">Seu site pronto em <span className="text-blue-600">5 Etapas</span></h2>
          <div className="grid md:grid-cols-5 gap-6">
            {[
              ...
              <motion.div key={i} variants={stepVariants} initial="offscreen" whileInView="onscreen" className="bg-slate-50 p-10 rounded-[2.5rem] relative border border-slate-100 min-h-[320px] flex flex-col justify-center shadow-sm">
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            <motion.div whileHover={{ y: -15, scale: 1.02 }} className="bg-white p-10 rounded-[3rem] border border-slate-200 flex flex-col hover:shadow-2xl transition-all">
              ...
              <div className="mb-8 font-black text-slate-900"><span className="text-2xl">R$</span> <span className="text-6xl tracking-tighter">700</span></div>
            ...
            <motion.div whileHover={{ y: -15, scale: 1.02 }} className="bg-slate-900 p-10 rounded-[3rem] text-white flex flex-col hover:shadow-2xl transition-all relative overflow-hidden shadow-xl shadow-blue-500/10">
              <div className="absolute top-8 right-8 bg-blue-600 text-[10px] font-black px-4 py-1 rounded-full uppercase tracking-widest">Mais Completo</div>
              ...
              <div className="mb-8 font-black"><span className="text-2xl text-blue-400">R$</span> <span className="text-6xl tracking-tighter">1.997</span></div>
              <div className="space-y-4 mb-10 flex-grow text-left">
        ...
        <div className="container mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-8">
          {[
            ...
            <div key={i} className="flex flex-col items-center p-8 bg-slate-50 rounded-[2rem] border border-slate-100 shadow-sm">
              ...
              <h4 className="font-bold text-slate-800 mb-2 uppercase tracking-tighter text-sm">{selo.t}</h4>
